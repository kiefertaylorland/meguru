...
func ToHiragana(input string) string {
	...
	for i := 0; i < n; {
		// n' disambiguator: forces the preceding "n" (already emitted as ん
		// by the fallback below) to stand alone rather than extend into a
		// na-row/nya-row mora. The apostrophe itself produces no output.
		if runes[i] == '\'' {
			i++
			continue
		}
