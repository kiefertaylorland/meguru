...
**Trust boundary:** the OS user account. A hostile process running *as the same user* can read anything that user can; defending against that is explicitly out of scope (no local crypto theater). Defending against accidental exposure — logs, shell history, world-readable files, oversharing to providers — is in scope.
